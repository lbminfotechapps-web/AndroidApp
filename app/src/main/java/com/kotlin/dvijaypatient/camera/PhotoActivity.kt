...
class PhotoActivity : AppCompatActivity() {
    ...
    private var cameraFlag: Int = 2 // Default is both cameras
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Retrieve the camera flag from the intent
        cameraFlag = intent.getIntExtra("CAMERA_FLAG", 2)
        //ActHome.Flag = 1
       // MainActivity.Flag=1
   ...
   /* private fun applyUserConfiguration() {
    ...
    private fun applyUserConfiguration() {
        ...
        lensFacing = when (cameraFlag) {
            0 -> CameraSelector.LENS_FACING_FRONT // Use front camera only
            1 -> CameraSelector.LENS_FACING_BACK  // Use back camera only
    ...
    private fun drawCameraControls() {
       ...
       /* controls.findViewById<ImageButton>(R.id.camera_switch_button).setOnClickListener {
        ...
        controls.findViewById<ImageButton>(R.id.camera_switch_button).setOnClickListener {
            lensFacing = when (cameraFlag) {
                0 -> CameraSelector.LENS_FACING_FRONT // Force front camera
                1 -> CameraSelector.LENS_FACING_BACK  // Force back camera
                2 -> { // Allow both cameras
                ...
                else -> lensFacing // Default to current lensFacing
            }

            // Re-bind all use cases with the updated lensFacing
            bindCameraUseCases()
        }

// Hide the camera switch button if only one camera is allowed
        ...
        //Added by vishal
        var isEnableFlash = true //By default we have set to auto mode
        controls.findViewById<ImageView>(R.id.ivFlash).setOnClickListener { ivFlash ->
            camera?.let {
                if (it.cameraInfo.hasFlashUnit()) {
                    isEnableFlash = !isEnableFlash //Toggle switch

                    imageCapture?.let { imageCapture ->
                        imageCapture.flashMode =
                            if (isEnableFlash) ImageCapture.FLASH_MODE_AUTO else ImageCapture.FLASH_MODE_OFF
                    }

                    (ivFlash as ImageView).setImageResource(if (isEnableFlash) R.drawable.ic_round_flash_auto_24 else R.drawable.ic_round_flash_off_24)
                }
            }
        }
    ...
    private fun bindCameraUseCases() = viewFinder.post {
        ...
        cameraProvider.addListener(Runnable {
            ...
            camera?.let { it ->
                if (it.cameraInfo.hasFlashUnit()) {
                    it.cameraInfo.torchState.observe(this) {
                        Log.i(TAG, "Flash state: $it")
                    }
                }
            }
